...
{
    internal static class AttributesForResolverFactory
    {
        public static GrpcAttributes GetAttributes()
        {
            var synchronizationContext = new GrpcSynchronizationContext((ex) => { });
            var attributes = GrpcAttributes.Builder.NewBuilder()
                .Add(GrpcAttributesConstants.DefaultLoadBalancingPolicy, "pick_first")
